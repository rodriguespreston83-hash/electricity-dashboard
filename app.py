...
index 881012518c6703471431ad07ad635dee3e76bd91..8c2b63fc7dee8883ccd80d27902343fbbfd8f8ff 100644
--- a/app.py
+++ b/app.py
@@ -1,269 +1,192 @@
...
+    .kpi-value { color: #f8fafc; font-size: 1.6rem; font-weight: 700; line-height: 1.2; }
...
+)
+
+PLOT_THEME = dict(template="plotly_dark", color_discrete_sequence=["#38bdf8", "#f59e0b", "#22c55e", "#ef4444", "#a855f7"])
+
 TARIFFS = {
...
-        )
-
+        df["cost"] = np.where(df["period"] == "Night", df["kWh"] * tariff["night"], df["kWh"] * tariff["day"])
     return df
...
-)
-
+df = apply_tariff(load_data(uploaded), tariff, night_start, night_end)
...
+    fig1 = px.area(daily, x="date", y="kWh", title="Daily Consumption", **PLOT_THEME)
...
-
+        pivot = grouped.pivot(index="dow", columns="hour", values="kWh").reindex([0, 1, 2, 3, 4, 5, 6])
+        pivot.index = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
...
-    )
-
+    fig = go.Figure(go.Heatmap(z=pivot.fillna(0).values, x=[f"{h:02d}:00" for h in pivot.columns], y=pivot.index, colorscale="Plasma"))
+    fig.update_layout(title=title, height=500, paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#cbd5e1"))
...
+    donut = px.pie(period_summary, values="kWh", names="period", hole=0.55, title="Usage Share by Tariff Period", **PLOT_THEME)
+    st.plotly_chart(donut, use_container_width=True)
+    bars = px.bar(period_summary, x="period", y="cost", title="Cost by Tariff Period", text_auto=".2f", **PLOT_THEME)
+    st.plotly_chart(bars, use_container_width=True)
+
+with tab4:
+    st.subheader("📊 7-Day Forecast")
+    avg = daily["kWh"].tail(14).mean()
+    future_dates = pd.date_range(pd.to_datetime(daily["date"].max()) + pd.Timedelta(days=1), periods=7)
+    forecast = pd.DataFrame({"date": future_dates, "kWh": avg})
+    if tariff["type"] == "standard":
...
+        forecast["cost"] = forecast["kWh"] * (0.6 * tariff["day"] + 0.4 * tariff["night"]) + tariff["standing"]
 
-st.plotly_chart(fig, use_container_width=True)
+    figf = go.Figure()
+    figf.add_trace(go.Scatter(x=daily["date"], y=daily["kWh"], name="Actual", mode="lines"))
+    figf.add_trace(go.Scatter(x=forecast["date"], y=forecast["kWh"], name="Forecast", mode="lines+markers"))
+    figf.update_layout(template="plotly_dark", title="Usage Forecast", yaxis_title="kWh")
